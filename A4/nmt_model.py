...
class NMT(nn.Module):
    ...
    def encode(self, source_padded: torch.Tensor, source_lengths: List[int]) -> Tuple[torch.Tensor, Tuple[torch.Tensor, torch.Tensor]]:
        ...
        # print(X.shape)
        src_packed = pack_padded_sequence(X, source_lengths)
        # pass the input sequence through encoder to get the hidden and cell states used to initialize decoder
        enc_hiddens, (last_hidden , last_cell) = self.encoder(src_packed)
        enc_hiddens, _ = pad_packed_sequence(enc_hiddens)
        enc_hiddens = enc_hiddens.permute(1, 0, 2)

        # compute decoder's first states using encoder's states
    ...
    def decode(self, enc_hiddens: torch.Tensor, enc_masks: torch.Tensor,
        ...
        for Y_t in torch.split(Y, 1):
            ...
            # print(Ybar_t.shape)
            dec_state, c_o, e_t  = self.step(Ybar_t, dec_state, enc_hiddens, enc_hiddens_proj, enc_masks)
            # print(o_t.shape)
            combined_outputs.append(c_o)
            o_prev = c_o
