...
def naiveSoftmaxLossAndGradient(
    ...
    scores =  softmax(np.dot(centerWordVec, outsideVectors.T))[:, np.newaxis]
    loss = -np.log(scores[outsideWordIdx])
    print(loss)
    gradCenterVec = -outsideVectors[outsideWordIdx] + np.sum(outsideVectors * scores, axis=0)
    gradOutsideVecs = np.dot(scores, centerWordVec[:, np.newaxis].T)
...
def negSamplingLossAndGradient(
    ...
    # Define reused parameters
    scores = outsideVectors.dot(centerWordVec)
    sigma_uo_t_vc = sigmoid(scores[outsideWordIdx])
    sigma_minus_uk_t_vc = sigmoid(- scores[negSampleWordIndices])
    loss = - np.log(sigma_uo_t_vc) - np.sum(np.log(sigma_minus_uk_t_vc))

    gradOutsideVecs = np.zeros(outsideVectors.shape)
    gradOutsideVecs[outsideWordIdx] = centerWordVec * (sigma_uo_t_vc - 1)
    gradOutsideVecs[negSampleWordIndices] =  np.dot((1 - sigma_minus_uk_t_vc)[:, np.newaxis], centerWordVec[:, np.newaxis].T)
    # Axis = 0 is critical since we want the sum along the columns only not for the whole matrix
    gradCenterVec = (sigma_uo_t_vc - 1) * outsideVectors[outsideWordIdx] \
        + np.sum(outsideVectors[negSampleWordIndices] * (1 - sigma_minus_uk_t_vc[:, np.newaxis]), axis=0)
    ### Please use your implementation of sigmoid in here.
    countIndices = np.bincount(negSampleWordIndices)
    for i in set(negSampleWordIndices):
        gradOutsideVecs[i] = gradOutsideVecs[i] * countIndices[i]
...
def skipgram(currentCenterWord, windowSize, outsideWords, word2Ind,
    ...
    ctr_w_vec = centerWordVectors[word2Ind[currentCenterWord]]
    for outWord in outsideWords:
        print(outWord)
        print(word2Ind[outWord])
        w_loss, w_gradCenter, w_gradOutside = word2vecLossAndGradient(
            ctr_w_vec,
            word2Ind[outWord],
            outsideVectors,
            dataset
        )
        gradOutsideVectors += w_gradOutside
        gradCenterVecs[word2Ind[currentCenterWord]] += w_gradCenter
        loss += w_loss
