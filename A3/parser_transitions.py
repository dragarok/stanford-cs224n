...
def minibatch_parse(sentences, model, batch_size):
    ...
    while unfinished_parses:
        # print([x.stack for x in unfinished_parses[:batch_size]]
        transitions = model.predict(unfinished_parses[:batch_size])
        for pobj, t in zip(unfinished_parses[:batch_size], transitions):
            pobj.parse_step(t)
        utestnfinished_parses[:] = [pobj for pobj in unfinished_parses if (pobj.buffer or (not len(pobj.stack) == 1))]

    dependencies = [pobj.dependencies for pobj in partial_parses]
    ### END YOUR CODE
