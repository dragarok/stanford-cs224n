...
class SynthesizerAttention(nn.Module):
    ...
    def forward(self, x, layer_past=None):
        ...
        v = self.value(x).view(B, T, self.n_head, hs).transpose(1, 2) # (B, nh, hs, T)
        xai = self.w1(x).view(B, T, self.n_head , hs).transpose(1, 2) # (B, nh, hs, T)
        relu_layer = nn.ReLU()
        att = relu_layer(xai) # (B, nh, hs, hs)
        att = (att @ self.w2) + self.b2 # (B, nh, hs, T)
