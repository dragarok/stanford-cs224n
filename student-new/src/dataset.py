...
class CharCorruptionDataset(Dataset):
    ...
    def truncate(self, document):
        chars = list(document)
        # sometimes block size's 7/8 can be larger than the document. Thus the min
        random_len = min(random.randint(4, int(self.block_size) * 7 / 8), len(chars))
        random_start = random.randint(0, len(chars) - random_len)
        random_idx = range(random_start, random_start + random_len)
        truncated = [chars[i] for i in random_idx]
    ...
    def __getitem__(self, idx):
        ...
        truncated = self.truncate(document)
        # get the first index and second index for the split
        random_split_1 = random.randint(0, int(len(truncated)/2))
        random_split_2 = random.randint(int(len(truncated)/2), int(len(truncated)))
        prefix = truncated[0:random_split_1]
        masked_content  = truncated[random_split_1:random_split_2]
        suffix = truncated[random_split_2:]

        # add mask characters in between for prediction
        ...
        # predicting character wise means for first character prediction is the second and so on.
        x = masked_string[:-1]
        y = masked_string[1:]
        x = torch.tensor([self.stoi[c] for c in x], dtype=torch.long)
        y = torch.tensor([self.stoi[c] for c in y], dtype=torch.long)
